...
namespace ClashOfBlocks
{
    public partial class FrmSettings : Form
    {
        ...
        {
            if (txtBoxNPass.TextLength >= 4 && txtBoxNPass.Text == txtBoxRePass.Text)
            {
                string mainconn = ConfigurationManager.ConnectionStrings["Myconnection"].ConnectionString;
                SqlConnection sqlconn = new SqlConnection(mainconn);
            ...
            else if(txtBoxNPass.TextLength < 4)
            {
                MessageBox.Show("Nova sifra mora imati minimum 4 karaktera");
                txtBoxNPass.Focus();
                return;
            }
        }
        ...
        private void TxtBoxNPass_Leave(object sender, EventArgs e)
        {
            ...
            else if (!rPass.IsMatch(txtBoxNPass.Text))
            {
                MessageBox.Show("Uneta nova sifra nije validna");
                txtBoxNPass.Focus();
                return;
            }
        }
